...
import WorkExperiencePackageTable from "@/app/manage/mentor/work-experience-package/package-table";
...
export default function WorkExperiencePackageForm() {
  const router = useRouter();
  const { data, isLoading, isError } = useWorkExpPackages();
    ...
    <div className="mx-auto min-w-7xl p-6">
      <Card className="rounded-md shadow-sm border border-gray-200">
        <CardHeader className="flex flex-row items-center justify-between bg-blue-500 text-white dark:bg-black p-6 rounded-t-md">
          <div className="flex items-center gap-4">
            ...
            <div>
              <CardTitle className="text-xl font-semibold">
                Work Experience Package Management
              </CardTitle>
              <p className="text-sm opacity-90">
                Manage your work experience packages efficiently
              </p>
            </div>
          </div>
          ...
          <Button
            onClick={() =>
              router.push("/manage/mentor/work-experience-package/create")
            }
        ...
        <CardContent className="p-6">
          ...
          {data?.payload?.data?.packages && (
            <WorkExperiencePackageTable data={data.payload.data.packages} />
          )}
        </CardContent>
      </Card>
    </div>
