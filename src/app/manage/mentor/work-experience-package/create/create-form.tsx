...
export default function CreateWorkExpPackageFormPage() {
  ...
  const { mutateAsync, isPending } = useCreateWorkExpPackage();
  const { data: coursesData, isLoading: coursesLoading } = useCourseByMentor();
  const [skills, setSkills] = useState<string[]>([]);
  const [skillInput, setSkillInput] = useState("");
  const profile = useProfile();

  const form = useForm<WorkExperiencePackageCreateInput>({
    resolver: zodResolver(WorkExperiencePackageCreateSchema),
    defaultValues: {
      mentorProfileId: profile?.mentor_profile_id as any,
      title: "",
      description: "",
      skills: [],
      duration: 1,
  ...
  const onSubmit = async (values: WorkExperiencePackageCreateInput) => {
    try {
      ...
      toast({
        title: "Work Experience Package created",
        ...
        <CardHeader className="bg-blue-500 text-white border-b rounded-t-md">
          <div className="flex items-center gap-3">
            ...
            <CardTitle className="text-xl font-semibold">
              Create Work Experience Package
            </CardTitle>
          </div>
        </CardHeader>
