...
import { useDeleteWorkExpPackage } from "@/queries/useWorkExpPackage";
import { getWorkExperiencePackageColumns } from "./columns";
...
export default function WorkExperiencePackageTable({
  data,
}: {
  data: WorkExperiencePackage[];
}) {
  const deletePackageMutation = useDeleteWorkExpPackage();
  const router = useRouter();

  const onDelete = async (workPackage: WorkExperiencePackage) => {
    const result = await Swal.fire({
      title: `Are you sure to delete "${workPackage.title}"?`,
    ...
    try {
      await deletePackageMutation.mutateAsync(workPackage.id);
      toast({
        title: "Package deleted",
        description: `"${workPackage.title}" was removed.`,
      });
    } catch (error) {
      toast({
        title: "Error",
  ...
  const columns = useMemo<ColumnDef<WorkExperiencePackage>[]>(
    () => getWorkExperiencePackageColumns(onDelete),
    []
  );
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = "work-experience-packages.csv";
    a.click();
                  ...
                  <div className="flex flex-col items-center justify-center gap-4 text-center">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-gray-500 max-w-md">
                        {search
                          ? "Try adjusting your search terms or create a new package."
                          : "Create your first work experience package to get started."}
                      </p>
                    </div>
                    <Button
                      onClick={() =>
                        router.push(
                          "/manage/mentor/work-experience-package/create"
                        )
                      }
