...
import { getSingleSessionColumns } from "./columns";
...
interface SingleSessionTableProps {
  data: SingleSessionType[];
...
export default function SingleSessionTable({
  ...
  onEdit,
  onAddNew,
}: SingleSessionTableProps) {
  const deleteSingleSessionMutation = useDeleteSingleSession();

  const onDelete = async (session: SingleSessionType) => {
    const result = await Swal.fire({
      title: `Are you sure to delete "${session.title}"?`,
    ...
    try {
      await deleteSingleSessionMutation.mutateAsync(session.id);
      toast({
  ...
  const columns = useMemo<ColumnDef<SingleSessionType>[]>(
    () => getSingleSessionColumns(onEdit, onDelete),
    [onEdit]
  );
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = "single-sessions.csv";
    a.click();
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-gray-500 max-w-md">
                        {search
                          ...
                          : "Create your first single session to get started."}
                      </p>
                    </div>
                    <Button
                      onClick={onAddNew}
