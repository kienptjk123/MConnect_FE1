...
import CourseTable from "@/app/manage/mentor/courses/course/course-table";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Separator } from "@/components/ui/separator";
import { useCourseByMentor, useCourses } from "@/queries/useCourse";
import { BookOpen, Plus } from "lucide-react";
import { useRouter } from "next/navigation";

export default function CourseForm() {
  const router = useRouter();
  const { data, isLoading, isError } = useCourseByMentor();
    ...
    <div className="mx-auto min-w-7xl p-6">
      <Card className="rounded-md shadow-sm border border-gray-200">
        <CardHeader className="flex flex-row items-center justify-between bg-blue-500 text-white dark:bg-black p-6 rounded-t-md">
          ...
          <Button
            onClick={() => router.push("/manage/mentor/courses/course/create")}
