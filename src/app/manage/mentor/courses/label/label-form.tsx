...
export default function LabelForm() {
  ...
  const { data, isLoading, isError } = useLabelsQuery();
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm opacity-90">
                Manage course labels efficiently
              </p>
            </div>
          </div>

          <Button
            onClick={() => router.push("/manage/mentor/courses/label/create")}
          ...
          {isError && (
            <div className="py-16 text-center">
              <div className="inline-flex flex-col items-center gap-3 text-red-600">
                ...
                <span>Failed to load labels. Please try again.</span>
              </div>
            </div>
          )}
          {Array.isArray(data?.payload?.data) && (
            <LabelTable data={data.payload.data} />
          )}
        </CardContent>
      </Card>
    </div>
