...
import { getLabelColumns } from "@/app/manage/mentor/courses/label/columns";
...
import { useDeleteLabelMutation } from "@/queries/useLabel";
...
export default function LabelTable({ data }: { data: LabelType[] }) {
  const deleteLabelMutation = useDeleteLabelMutation();
  const router = useRouter();

  const onDelete = async (label: LabelType) => {
    ...
    try {
      ...
      toast({
        title: "Label deleted",
        description: `"${label.name}" was removed.`,
      });
    } catch (error) {
      toast({
        title: "Error",
  ...
  const columns = getLabelColumns(onDelete);
    ...
    <div className="w-full space-y-4">
      ...
      <div className="flex items-center justify-between px-2">
        <div className="flex-1 text-sm text-muted-foreground">
          {table.getFilteredRowModel().rows.length} total labels
        </div>
