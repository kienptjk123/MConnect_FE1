...
import { useDeleteStaffMutation } from "@/queries/useStaff";
...
export default function StaffTable({ data }: { data: StaffType[] }) {
  const deleteStaffMutation = useDeleteStaffMutation();
  const router = useRouter();

  const onDelete = async (staff: StaffType) => {
    ...
    try {
      ...
      toast({
        title: "Staff deleted",
        description: `"${staff.name}" was removed.`,
      });
    } catch (error) {
      toast({
        title: "Error",
  ...
  const columns = useMemo<ColumnDef<StaffType>[]>(
    () => getStaffColumns(onDelete),
    []
  );
              ...
              <TableRow>
                ...
                >
                  <div className="flex flex-col items-center justify-center gap-4 text-center">
                    ...
                    <Button
                      onClick={() =>
                        router.push("/manage/admin/manage-staff/create")
                      }
